...
const cliPath = path.join(packageRoot, 'bin/code-anchored-context-reference.js');
const packageJson = JSON.parse(await readFile(path.join(packageRoot, 'package.json'), 'utf8'));

test('init installs the reference scaffold and skills', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'cac-reference-'));
  ...
  assert.match(stdout, /Release-Anchored Reference ready for Reference App/);
  ...
  assert.match(agents, /<!-- code-anchored-context:reference:start -->/);
  ...
  // The reference package does not install planning machinery.
  assert.equal(await exists(path.join(target, 'context')), false);
  assert.equal(await exists(path.join(target, 'decisions')), false);

  const metadata = JSON.parse(
    await readFile(path.join(target, '.code-anchored-context/reference.json'), 'utf8')
  );
...
test('init preserves existing docs and an existing AGENTS file', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'cac-reference-existing-'));
  ...
  assert.match(stdout, /AGENTS\.md already has the Release-Anchored Reference guidance/);
