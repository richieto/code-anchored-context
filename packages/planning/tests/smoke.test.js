...
const cliPath = path.join(packageRoot, 'bin/code-anchored-context-planning.js');
const packageJson = JSON.parse(await readFile(path.join(packageRoot, 'package.json'), 'utf8'));

test('init installs the flat planning scaffold and decision log', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'cac-planning-'));
  ...
  assert.match(stdout, /Plan With Durable Context ready for Planning App/);
  ...
  assert.equal(
    await exists(path.join(target, '.agents/skills/grill-and-distribute/SKILL.md')),
    true
  );

  // The flat model has no release folders or current.md.
  ...
  assert.equal(await exists(path.join(target, 'context/programs')), false);

  const agents = await readFile(path.join(target, 'AGENTS.md'), 'utf8');
  assert.match(agents, /# Agent Guidance - Planning App/);
  assert.match(agents, /<!-- code-anchored-context:planning:start -->/);
  assert.match(agents, /\.agents\/skills\/plan-with-context\/SKILL\.md/);
  assert.match(agents, /\.agents\/skills\/grill-and-distribute\/SKILL\.md/);
  ...
  assert.doesNotMatch(profile, /Release Closeout Profile/);

  const decisions = await readFile(path.join(target, 'decisions/README.md'), 'utf8');
  assert.match(decisions, /append-only decision log for Planning App/);

  const metadata = JSON.parse(
    await readFile(path.join(target, '.code-anchored-context/planning.json'), 'utf8')
  );
  assert.equal(metadata.installedVersion, packageJson.version);
  assert.equal(metadata.projectName, 'Planning App');
  assert.deepEqual(metadata.installedSkills, ['plan-with-context', 'grill-and-distribute']);
  ...
  assert.match(status.stdout, /Installed skills: plan-with-context, grill-and-distribute/);
});

test('init appends planning guidance to an existing AGENTS file and is idempotent', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'cac-planning-existing-'));
  ...
  assert.match(stdout, /AGENTS\.md already has the Plan With Durable Context guidance/);
